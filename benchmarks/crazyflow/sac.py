...
def train(
    ...
    for i in range(n_runs):
        ...
        actor = SACActor(
            train_envs.single_observation_space.shape, train_envs.single_action_space.shape
        )
        critic = SACCritic(
            train_envs.single_observation_space.shape, train_envs.single_action_space.shape
        )
        ...
        eval_collector.append(
            LogCollector(target="reward", log_key="eval/steps", reduce="cnt")
        )
        config.sac.rollout_collector = rollout_collector
        config.sac.eval_collector = eval_collector

        sac(train_envs, eval_envs, **config.sac)
...
def main(
    env: str | None = None,
    action: Literal["quat", "matrix", "euler", "tangent"] = "tangent",
    ...
    """Train the SAC agent on the given crazyflow experiment.
