...
import torch
import wandb
from envs import ReachOrient, RotationWrapper
...
def train(
    ...
    save_dir = (
        ...
        / "saves/her/pick_orient"
        / config.wrapper.action
        / config.wrapper.control
    )
    wandb_config = config.to_dict()

    relative = config.wrapper.control == "rel"
    scale = None
    if "scale" in config.wrapper:
        scale = config.wrapper.scale
    elif config.wrapper.control == "rel" and config.wrapper.action == "tangent":
        scale = 0.1
    wrappers = [lambda env: RotationWrapper(env, config.wrapper.action, relative, rot_scale=scale)]
    env = gymnasium.make_vec(
        config.env.name,
        num_envs=config.env.n_envs,
    ...
    replay_buffer = HerVectorReplayBuffer(
        ...
        reward_fn=ReachOrient().compute_reward,
...
def main(
    ...
    """
    assert action in ["euler", "matrix", "quat", "tangent"], "Invalid action"
    assert control in ["abs", "rel"], "Invalid control"
    config = load_config(Path(__file__).parent / "config/pick_orient.toml")
    if group is not None:
        config.wandb.group = group
    else:
        config.wandb.group = f"{action}|{control}"
    config.wrapper = ConfigDict({"action": action, "control": control})
